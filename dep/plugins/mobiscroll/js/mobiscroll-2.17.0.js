...
(function ($, window, document, undefined) {
    ...
    ms.classes.Frame = function (el, settings, inherit) {
        ...
        that.select = function () {
            s.isSelected = true;    //dhj 新增 是否已经点击过确定
            if (isModal) {
        ...
        that.show = function (prevAnim, prevFocus) {
            ...
            //ly 新增 日期时间切换
            $('.dw-tab-data').on('tap click', function(e){
                if(!$('.mbsc-w-p').first().hasClass('mbsc-show')){
                    $('.mbsc-w-p').last().removeClass('mbsc-show').addClass('mbsc-hide');
                    $('.mbsc-w-p').first().removeClass('mbsc-hide').addClass('mbsc-show');
                    $('.dw-tab-data').addClass('dw-tab-selected');
                    $('.dw-tab-time').removeClass('dw-tab-selected');
                }
            });
            $('.dw-tab-time').on('tap click', function(e){
                if(!$('.mbsc-w-p').last().hasClass('mbsc-show')){
                    $('.mbsc-w-p').first().removeClass('mbsc-show').addClass('mbsc-hide');
                    $('.mbsc-w-p').last().removeClass('mbsc-hide').addClass('mbsc-show');
                    $('.dw-tab-time').addClass('dw-tab-selected');
                    $('.dw-tab-data').removeClass('dw-tab-selected');
                }
            });         
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        that._generateContent = function () {
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                var classTag = 'mbsc-show';
                if(s.wheels.length>1){
                    classTag = '';
                }
                if(settings.headerText && settings.headerText.indexOf('dw-tab-data')>0){
                    if(i === 0){
                        classTag = 'mbsc-show';
                    }else if(i === 1){
                        classTag = 'mbsc-hide';
                ...
                html += '<div class="mbsc-w-p dwc '+ classTag + ' ' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '">' +
    ...
    var ms = $.mobiscroll,
        ...
        preset = function (inst) {
            ...
            var i,
                ...
                defd = s.defaultValue || new Date(),    //dhj改 增加 s.defaultValue
                steps = s.steps || {},
                ...
                for (k = offset; k < offset + 4; k++) {
                    if (k == o.h) {
                        ...
                        for (i = minH; i < (hampm ? 12 : 24); i += stepH) {
                            keys.push(i);
                            values.push((hampm && i === 0 ? 12 : tord.match(/hh/i) && i < 10 ? '0' + i : i) + (s.hourSuffix || ''));  //dhj改 增加'时'单位
                        }
                        ...
                        for (i = minM; i < 60; i += stepM) {
                            keys.push(i);
                            values.push((tord.match(/ii/) && i < 10 ? '0' + i : i) + (s.minuteSuffix || ''));  //dhj改 增加'分'单位
                        }
                        ...
                        for (i = minS; i < 60; i += stepS) {
                            keys.push(i);
                            values.push((tord.match(/ss/) && i < 10 ? '0' + i : i) + (s.secondSuffix || ''));  //dhj改 增加'秒'单位
                        }
...
(function ($, undefined) {
    var ms = $.mobiscroll,
        ...
        defaults = {
            ...
            invalid: [],
            rtl: false,
            showMe: false,    //dhj 新增 本身元素是否显示，默认不显示
            showInput: true,
            groupLabel: 'Groups',
            checkIcon: 'checkmark',
            dataText: 'text',
            ...
            dataSelected: 'selected',    //dhj 新增 被选择的属性
            isSelected: false     //dhj 新增 是否已经点击过确定
        };
    ...
    ms.presets.scroller.select = function (inst) {
        var change,
            ...
            defaultValue = s.defaultValue,      //dhj 改  
            batch = s.batch,
        ...
        if(!s.showMe){  //dhj 新增 判断条件为新增，如果showMe为false 则绑定的元素隐藏掉，否则显示
...
/*
* mobiscroll.frame.ios-classic.js
*/
(function ($) {
...
/*
* mobiscroll.android-holo-light.js
*/
(function ($) {
...
/*
* mobiscroll.frame.mobiscroll-dark.js
*/
(function ($) {
